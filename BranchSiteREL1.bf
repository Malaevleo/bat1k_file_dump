...
ChoiceList  (useErrorSink,"Enable error-sink component?", 1,NO_SKIP,
             "Off","[Default] Do not include error-sink component",
             "On","Include error-sink class with very large omega");

// If the user aborts or closes the dialog, exit
if (useErrorSink < 0) {
    return 1;
}

global do_error_sink = useErrorSink;  // 0 = Off, 1 = On
...
if (do_error_sink == 1) {
    // p_error_sink is the proportion of sites in the error-sink class
    global p_error_sink = 0.001;  // initial guess
    p_error_sink :> 0.0;         // lower bound
    p_error_sink :< 0.01;        // upper bound

    // omega_error_sink is the (log of the) large omega for the error-sink
    // Typically put on a log-scale to keep it in [ln(100), ln(1e10)]
    global omega_error_sink = 7;  // e^7 ~ 1100 (just an example)
    omega_error_sink :> ln(100);   // ln(100) ~ 4.605
    omega_error_sink :< ln(1e10);  // ln(1e10) ~ 23.026
}
...
for (matrix_id = 2; matrix_id <= maximum_number_of_omegas; matrix_id += 1) {
    ...
    // 1. Generate frequency weights with or without the error-sink
    freq_weights = generateFrequencyParameterization ("Paux", matrix_id, do_error_sink);

    // 2. Multiply the “normal” categories (indices 0..matrix_id-1) by Exp(MGMatrixX)
    for (k = 0; k < matrix_id; k += 1) {
        freq_weights[k] = "Exp(MGMatrix" + (k+1) + ")*" + freq_weights[k];
    }

    // 3. If do_error_sink == 1, the last entry is p_error_sink; multiply it by Exp(omega_error_sink)
    if (do_error_sink == 1) {
        freq_weights[matrix_id] = "Exp(omega_error_sink)*" + freq_weights[matrix_id];
    }

    // 4. Build the “MG” model by summing all mixture categories
    ExecuteCommands ("Model MG" + matrix_id + "=(\"" + Join("+", freq_weights) 
                    + "\",codon3x4,EXPLICIT_FORM_MATRIX_EXPONENTIAL);");
}
...
lfunction printNodeDesc (ref, rate_classes) {
    if (rate_classes > 1) {
        ...
        wts = generateFrequencyParameterization ("Paux", rate_classes, do_error_sink);
...
lfunction generateFrequencyParameterization (prefix, count, has_error_sink) {
    /*
     * If has_error_sink == 0:
     *   This function behaves exactly as before (no error-sink).
     *
     * If has_error_sink != 0:
     *   We produce 'count+1' total categories:
     *     - The first 'count' categories sum to (1 - p_error_sink).
     *     - The final category is p_error_sink (the "error-sink").
     *
     * Note that you'll want to pass:
     *   generateFrequencyParameterization("Paux", matrix_id, do_error_sink)
     * in places where you want an extra error-sink category if do_error_sink==1.
     */
    ...
    if (has_error_sink == 0) {
        ...
        // ==========================
        // NEW CODE WITH ERROR-SINK
        // ==========================
        // We'll add one extra category at the end for p_error_sink.
        // The first 'count' categories get (1 - p_error_sink).

        // Create a result of dimension 1 row x (count+1) columns.
        result = {1, count+1};

        // If count>1, we do the standard logistic trick but scaled by (1 - p_error_sink).
        if (count > 1) {
            result[0] = "(1-p_error_sink)*`prefix`1";
            buffer = "(1-p_error_sink)*(1-`prefix`1)";
            for (i = 1; i < count - 1; i+=1) {
                result[i] = buffer + "*" + "`prefix`" + (i+1);
                buffer   += "*(1-`prefix`" + (i+1) + ")";
            }
            ...
            // If count == 1, that single category is just (1 - p_error_sink).
            result[0] = "(1-p_error_sink)";
        }

        // The last (error-sink) category is p_error_sink
        result[count] = "p_error_sink";
...
if (do_error_sink == 1) {
    fprintf (stdout, "\n[MODEL INFO] An error-sink category HAS BEEN ENABLED.\n");
    fprintf (stdout, "            * p_error_sink       = ", p_error_sink, "\n");
    fprintf (stdout, "            * omega_error_sink   = ", Exp(omega_error_sink), " (i.e., exp(", omega_error_sink, "))\n");
} else {
    fprintf (stdout, "\n[MODEL INFO] No error-sink category is included.\n");
}

// Next, also log to JSON. 
// For example, you might want to store it under the 'fits' : 'Adaptive Model' node.

if (do_error_sink == 1) {
    // Typically, your JSON object is called _BSREL_json, e.g. 
    // _BSREL_json[terms.json.fits][_BSREL.adaptive_model] = { ... } 
    // We'll just append a field named "error_sink" or similar.

    (
      (_BSREL_json[terms.json.fits])
      [_BSREL.adaptive_model]
    )["error_sink"] = {
        "enabled" : 1,
        "p_error_sink" : p_error_sink,
        // We can store the exponentiated value of omega_error_sink 
        // (i.e., the actual w ~ [100..1e10]) or just the log value
        "omega_error_sink" : Exp(omega_error_sink)
    };
} else {
    (
      (_BSREL_json[terms.json.fits])
...
// That’s enough to inform both the user (console) and 
// any programmatic consumers (JSON) that you used (or didn’t use) the error-sink.
