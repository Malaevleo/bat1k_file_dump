...
KeywordArgument("error-sink", "Add an additional 'error sink' omega per branch?", "Off");
absrel.error_sink = io.SelectAnOption(
    {
        "On" : "Add an error sink class to each branch",
        "Off"  : "[Default] do not add an error sink"
    },
    "Would you like to fit an additional error omega on each branch?"
);

selection.io.json_store_setting  (absrel.json, "error-sink", absrel.error_sink);
...
for (absrel.branch_id = 0; absrel.branch_id < absrel.branch_count; absrel.branch_id += 1) {
    ...
    absrel.current_rate_count = 2; // Start with 2 rate classes
    ...
    while (TRUE) {
        ...
        // Apply constraints based on the current rate count and error sink
        absrel.SetBranchConstraints(absrel.model_defintions[absrel.current_rate_count], absrel.tree_id, absrel.current_branch);

        // Optimization step with grid search
        Optimize(absrel.stepup.mles, ^absrel.likelihood_function_id, {
            "OPTIMIZATION_METHOD": "nedler-mead",
            "OPTIMIZATION_PRECISION": 1e-4,
            "OPTIMIZATION_START_GRID": absrel.ComputeOnAGrid(absrel.PopulateInitialGrid(
                absrel.model_defintions[absrel.current_rate_count],
                absrel.tree_id,
                absrel.current_branch,
                absrel.current_branch_estimates
            ), absrel.likelihood_function_id)
        });

        // Compute AICc and compare with the current best score
        absrel.current_test_score = math.GetIC(
            absrel.stepup.mles[1][0],
            absrel.current_parameter_count + 2,
            absrel.codon_data_info[terms.data.sample_size]
        );

        absrel.provisional_estimates = absrel.GetBranchEstimates(
            absrel.model_defintions[absrel.current_rate_count],
            absrel.tree_id,
            absrel.current_branch
        );
        ...
        // Format report for the current rate class
        if (absrel.dn_ds.distro[absrel.current_rate_count - 1][0] < 1000) {
            absrel.report.row[3] = Format(
                absrel.dn_ds.distro[absrel.current_rate_count - 1][0], 5, 2
            ) + " (" + Format(
                absrel.dn_ds.distro[absrel.current_rate_count - 1][1] * 100, 5, 2
            ) + "%)";
        } else {
            absrel.report.row[3] = ">1000 (" + Format(
                absrel.dn_ds.distro[absrel.current_rate_count - 1][1] * 100, 5, 2
            ) + "%)";
        }
        ...
        // Include multiple-hit rates if applicable
        if (absrel.multi_hit == "Double" || absrel.multi_hit == "Double+Triple") {
            absrel.report.row[4] = (
                absrel.provisional_estimates[utility.getGlobalValue("terms.parameters.multiple_hit_rate")]
            )[utility.getGlobalValue("terms.fit.MLE")];
            absrel.offset = 1;
        }
        if (absrel.multi_hit == "Double+Triple") {
            absrel.report.row[5] = (
                absrel.provisional_estimates[utility.getGlobalValue("terms.parameters.triple_hit_rate")]
            )[utility.getGlobalValue("terms.fit.MLE")];
            absrel.offset += 1;
        }
        ...
        // Decide whether to keep the new rate class or revert
    ...
    // Reset the model if the test score did not improve
    if (absrel.current_test_score >= absrel.current_best_score) {
        absrel.current_rate_count -= 1;
        if (absrel.current_rate_count >= 2) {
            model.ApplyToBranch(
                (absrel.model_defintions[absrel.current_rate_count])[terms.id],
                absrel.tree_id,
                absrel.current_branch
            );
            absrel.SetBranchConstraints(
                absrel.model_defintions[absrel.current_rate_count],
                absrel.tree_id,
                absrel.current_branch
            );
    ...
    // Add current estimates to the full model parameters
    utility.AddToSet(
        absrel.full_model_parameters,
...
lfunction absrel.PopulateInitialGrid (model, tree_id, branch_id, current_estimates) {
    ...
    if (component_count == 2) {
        ...
        if (absrel.error_sink == "On") {
            omega_prev_index = component_count - 2;
        } else {
            omega_prev_index = component_count - 1;
        }
        omega_prev_key = terms.AddCategory(utility.getGlobalValue('terms.parameters.omega_ratio'), omega_prev_index);
        if (None != current_estimates[omega_prev_key]) {
            omega_prev = current_estimates[omega_prev_key];
            if (omega_prev[utility.getGlobalValue('terms.fit.MLE')] > 1) {
                parameters.SetValue(omega_prev[utility.getGlobalValue('terms.id')], 0.8);
            }
        } else {
            console.log("Warning: omega_prev_key not found for component_count =", component_count);
        }
...
lfunction absrel.codon.BS_REL._DefineQ.TH (bs_rel, namespace) {
    ...
    // This sets up the function pointer to compute rates for each matrix entry
    bs_rel [utility.getGlobalValue("terms.model.q_ij")] = &rate_generator;

    // This will hold the mixture keys (component_1, component_2, etc.)
    bs_rel [utility.getGlobalValue("terms.mixture.mixture_components")] = {};

    // How many categories do we have now (possibly +1 if error_sink is on)?
    component_count = bs_rel[utility.getGlobalValue("terms.model.components")];

    // Generate local parameter names for stick-breaking if we have multiple categories
    ...
    // Convert those local param names into actual mixture weights
    ...
    for (component = 1; component <= component_count; component += 1) {
       ...
       /***********************************************************
        1) If this is the LAST component AND error_sink == "On",
           define a special rate generator with 'omega_error'
        ***********************************************************/
       if (component == component_count && ^"absrel.error_sink" == "On") {
         ...
         // The NORMAL scenario (no error sink), or not the last category
         ExecuteCommands ("
            function rate_generator (fromChar, toChar, namespace, model_type, model) {
               return models.codon.MG_REV_TRIP._GenerateRate_generic (
                    fromChar, toChar, 
       ...
       // If this is NOT the last category, attach a local param for the mixture weight
    ...
    // Typically fix the A->G nucleotide rate to 1
    parameters.SetConstraint(
...
lfunction absrel.codon.BS_REL._DefineQ.DH (bs_rel, namespace) {
    ...
    // Possibly +1 if error-sink was enabled in the main script
    component_count = bs_rel[utility.getGlobalValue("terms.model.components")];
    ...
    for (component = 1; component <= component_count; component += 1) {
       ...
       // If last component + error_sink -> 'omega_error'
       if (component == component_count && ^"absrel.error_sink" == "On") {
       ...
       // Constrain the error sink 'omega_error' to [100,1e10]
       if (component == component_count && ^"absrel.error_sink" == "On") {
           error_omega_id = terms.AddCategory(utility.getGlobalValue('terms.parameters.omega_ratio'), component);
           parameters.SetRange(
               "`" + bs_rel[terms.id] + "`.`local_parameters[" + error_omega_id + "]`",
...
lfunction absrel.BS_REL._DefineQ(bs_rel, namespace) {
    ...
    // Generate auxiliary parameters for mixture weights
    _aux = parameters.GenerateSequentialNames(
        "bsrel_mixture_aux",
        component_count - 1,
        "_"
    ...
    for (component = 1; component <= component_count; component += 1) {
        ...
        // Define the rate generator for error-sink or normal components
        if (component == component_count && ^"absrel.error_sink" == "On") {
        ...
        // Add local parameters for non-error-sink components
        if (component < component_count) {
            model.generic.AddLocal(
                bs_rel,
                _aux[component - 1],
        ...
        // Define Q matrix and store it in rate_matrices
        models.codon.generic.DefineQMatrix(bs_rel, namespace);
        rate_matrices[key] = bs_rel[utility.getGlobalValue("terms.model.rate_matrix")];
        (bs_rel[^'terms.mixture.mixture_components'])[key] = _wts[component - 1];

        // Constrain 'omega_error' range if this is the error-sink component
        if (component == component_count && ^"absrel.error_sink" == "On") {
            error_omega_id = terms.AddCategory(utility.getGlobalValue('terms.parameters.omega_ratio'), component);
            parameters.SetRange(
                error_omega_id,,
    ...
    // Set the final rate matrix
    bs_rel[utility.getGlobalValue("terms.model.rate_matrix")] = rate_matrices;

    // Fix the A->G nucleotide rate to 1
    parameters.SetConstraint(
        ((bs_rel[utility.getGlobalValue("terms.parameters")])[utility.getGlobalValue("terms.global")])[terms.nucleotideRate("A", "G")],
        "1",
        ""
