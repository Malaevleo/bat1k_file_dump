...
KeywordArgument("error-sink", "Add an additional 'error sink' omega per branch?", "No");
...
absrel.error_sink = io.SelectAnOption(
    {
        "Yes" : "Add an error sink class to each branch",
        "No"  : "[Default] do not add an error sink"
    },
    "Would you like to fit an additional error omega on each branch?"
);
...
for (absrel.i = 2; absrel.i <= absrel.max_rate_classes; absrel.i += 1) {
    if (absrel.error_sink == "Yes") {
       // Because we want 1 extra error-sink category
       absrel.number_of_components = absrel.i + 1;
   } else {
       absrel.number_of_components = absrel.i;
   }
    absrel.model_defintions [absrel.i] = model.generic.DefineMixtureModel("absrel.BS_REL.ModelDescription",
            "absrel.model." + absrel.i, {
                ...
                "2": parameters.Quote (absrel.number_of_components) // the number of rate classes
            },
            absrel.filter_names,
            None);
...
lfunction absrel.codon.BS_REL._DefineQ.TH (bs_rel, namespace) {
    ...
    // partial stick-breaking for up to (component_count - 2)
    if (component_count > 2) {
        _aux = parameters.GenerateSequentialNames(
                   "bsrel_mixture_aux",
                   component_count - 2,
                   "_"
               );
    ...
    if (component_count > 1 && ^"absrel.error_sink" == "Yes") {
        error_fraction_name = "error_sink_fraction_TH";
        model.generic.AddLocal (
            bs_rel,
            error_fraction_name,
            terms.AddCategory (utility.getGlobalValue("terms.mixture.mixture_aux_weight"), component_count)
        );
        // Force [0,0.01]
        parameters.SetRange (
            error_fraction_name,
    ...
    for (component = 1; component <= component_count; component += 1) {
       ...
       // If last + error sink => 'omega_error'
       if (component == component_count && ^"absrel.error_sink" == "Yes") {

         ExecuteCommands ("
            function rate_generator (fromChar, toChar, namespace, model_type, model) {
               return models.codon.MG_REV_TRIP._GenerateRate_generic(
                    fromChar, toChar,
                    namespace,
                    model_type,
                    model[utility.getGlobalValue('terms.translation_table')],
                    'alpha',  utility.getGlobalValue('terms.parameters.synonymous_rate'),
                    'omega_error', terms.AddCategory(utility.getGlobalValue('terms.parameters.omega_ratio'), " + component + "),
                    'omega_error', terms.AddCategory(utility.getGlobalValue('terms.parameters.omega_ratio'), " + component + "),
                    'delta', utility.getGlobalValue('terms.parameters.multiple_hit_rate'),
                    'psi',   utility.getGlobalValue('terms.parameters.triple_hit_rate'),
                    'psi',   utility.getGlobalValue('terms.parameters.triple_hit_rate')
       ...
       // Mixture weight logic:
       if (component < component_count - 1) {
            // Normal partial stick-break
            model.generic.AddLocal (
                bs_rel,
                _aux[component - 1],
                terms.AddCategory (utility.getGlobalValue("terms.mixture.mixture_aux_weight"), component)
            );
       ...
       } else if (component == (component_count - 1) && ^"absrel.error_sink" == "Yes") {
            leftoverName = "partial_leftover_TH_" + component;
            model.generic.AddLocal (
                bs_rel,
                leftoverName,
                terms.AddCategory (utility.getGlobalValue("terms.mixture.mixture_aux_weight"), component)
            );
            // leftover = 1 - sumSoFar - error_sink_fraction
            ExecuteCommands ("
               function " + leftoverName + "() {
                   sumSoFar = 0;
                   for (i = 0; i < " + (component - 1) + "; i += 1) {
                       sumSoFar += ^" + _aux[0] + "[i];
                   }
                   return 1 - sumSoFar - ^" + error_fraction_name + ";
               }
            ");
            parameters.SetConstraint(leftoverName, leftoverName, "");
            (bs_rel [^'terms.mixture.mixture_components'])[key] = ^leftoverName;

       } else if (component == component_count && ^"absrel.error_sink" == "Yes") {
            // error sink fraction
            (bs_rel [^'terms.mixture.mixture_components'])[key] = ^error_fraction_name;

       } else {
            // fallback
            if (component_count == 1) {
                (bs_rel [^'terms.mixture.mixture_components'])[key] = 1;
            } else {
                (bs_rel [^'terms.mixture.mixture_components'])[key] = _wts[component - 1];
            }
       }
       ...
       // If error sink, constrain 'omega_error'
       if (component == component_count && ^"absrel.error_sink" == "Yes") {
           error_omega_id = terms.AddCategory(utility.getGlobalValue('terms.parameters.omega_ratio'), component);
           parameters.SetRange(
               model.generic.GetGlobalParameter (bs_rel, error_omega_id),
               {lower_bound : "100", upper_bound : "1e10"}
           );
       }
    }
    ...
    // Fix A->G to 1
    parameters.SetConstraint(
       ((bs_rel[utility.getGlobalValue("terms.parameters")])[utility.getGlobalValue("terms.global")])[
           terms.nucleotideRate("A","G")
       ],
       "1", 
       ""
    );
...
lfunction absrel.codon.BS_REL._DefineQ.DH (bs_rel, namespace) {
    ...
    // 1) Set up the function pointer for Q_ij
    bs_rel [utility.getGlobalValue("terms.model.q_ij")] = &rate_generator;

    // 2) This dict will hold "component_1", "component_2", ...
    bs_rel [utility.getGlobalValue("terms.mixture.mixture_components")] = {};

    // 3) Possibly +1 if user selected error sink
    component_count = bs_rel[utility.getGlobalValue("terms.model.components")];

    /*******************************************************************
     4) We generate stick-breaking params for (component_count - 2) 
        categories, if we have >=3 categories. 
        Because the last category is forced to [0,0.01].
    ********************************************************************/
    if (component_count > 2) {
        _aux = parameters.GenerateSequentialNames(
                    "bsrel_mixture_aux",
        ...
        // If we have only 2 categories total, there's no partial leftover
        _aux = {};
    }
    _wts = parameters.helper.stick_breaking (_aux, None);

    /*******************************************************************
     5) If we have an error sink, define a local param for its fraction
        in [0,0.01]
    ********************************************************************/
    if (component_count > 1 && ^"absrel.error_sink" == "Yes") {
        error_fraction_name = "error_sink_fraction_DH";
        model.generic.AddLocal (
            bs_rel,
            error_fraction_name,
            terms.AddCategory (utility.getGlobalValue("terms.mixture.mixture_aux_weight"), component_count)
    ...
    // 6) Main loop over mixture components
    for (component = 1; component <= component_count; component += 1) {

       key = "component_" + component;

       //---------------------------------------------------------------
       // If last comp + error sink => param name 'omega_error'
       //---------------------------------------------------------------
       if (component == component_count && ^"absrel.error_sink" == "Yes") {
         ExecuteCommands ("
            function rate_generator (fromChar, toChar, namespace, model_type, model) {
               return models.codon.MG_REV_MH._GenerateRate_generic(
                    fromChar, toChar,
                    namespace,
                    model_type,
                    model[utility.getGlobalValue('terms.translation_table')],
       ...
       //---------------------------------------------------------------
       // Assign mixture weight for this component
       //---------------------------------------------------------------
       // Case 1: If we have multiple components, but not the last or second-last
       if (component < component_count - 1) {
            // Normal stick-break param
            model.generic.AddLocal (
                bs_rel,
                _aux[component - 1],
                terms.AddCategory (utility.getGlobalValue("terms.mixture.mixture_aux_weight"), component)
            ...
            // Second-last category => leftover = 1 - sumSoFar - error_sink_fraction
            leftoverName = "partial_leftover_" + component;
            model.generic.AddLocal (
                bs_rel,
                leftoverName,
                terms.AddCategory (utility.getGlobalValue("terms.mixture.mixture_aux_weight"), component)
            );
            // We define a custom function to compute leftover:
            ExecuteCommands ("
               function " + leftoverName + "() {
                   sumSoFar = 0;
                   for (i = 0; i < " + (component - 1) + "; i += 1) {
            ...
            // The final category = error sink fraction
            (bs_rel [^'terms.mixture.mixture_components'])[key] = ^error_fraction_name;

       } else {
            // If there's no error sink or we have only 1 category
            // fallback to leftover
            if (component_count == 1) {
                (bs_rel [^'terms.mixture.mixture_components'])[key] = 1;
            } else {
                (bs_rel [^'terms.mixture.mixture_components'])[key] = _wts[component - 1];
       ...
       // 7) Build the Q matrix for this category
       models.codon.generic.DefineQMatrix(bs_rel, namespace);
       rate_matrices [key] = bs_rel[utility.getGlobalValue("terms.model.rate_matrix")];

       //---------------------------------------------------------------
       // If it's the error sink, force omega_error in [100, 1e10]
       //---------------------------------------------------------------
       if (component == component_count && ^"absrel.error_sink" == "Yes") {
           error_omega_id = terms.AddCategory(utility.getGlobalValue('terms.parameters.omega_ratio'), component);
           parameters.SetRange(
    ...
    // 9) Typically fix the A->G nucleotide rate to 1
    parameters.SetConstraint(
       ((bs_rel[utility.getGlobalValue("terms.parameters")])[utility.getGlobalValue("terms.global")])[
           terms.nucleotideRate("A","G")
       ],
...
lfunction absrel.BS_REL._DefineQ (bs_rel, namespace) {
    ...
    // Dictionary to hold per-component Q matrices
    rate_matrices = {};

    // The function pointer to compute entry-wise rates
    bs_rel [utility.getGlobalValue("terms.model.q_ij")] = &rate_generator;

    // Will store “component_1”, “component_2”, ... plus their mixture weights
    bs_rel [utility.getGlobalValue("terms.mixture.mixture_components")] = {};

    component_count = bs_rel[utility.getGlobalValue("terms.model.components")];

    /*******************************************************************
     1) We'll generate local param names for *component_count - 2* of them
        because we want to handle the LAST category's fraction manually
        plus 1 'leftover' category before it.
    ********************************************************************/
    if (component_count > 2) {
        // e.g. if we have 3 or more categories
        _aux = parameters.GenerateSequentialNames ("bsrel_mixture_aux",
                                                   component_count - 2,
                                                   "_");
    } else {
        // If we only have 2 categories total, we'll have 0 stick-breakers
        _aux = {};
    }

    // We'll convert these to partial mixture weights
    _wts = parameters.helper.stick_breaking(_aux, None);

    /*******************************************************************
     2) Create a local param for the *error sink fraction* directly.
        We will call it 'error_sink_fraction', constrained to [0,0.01].
    ********************************************************************/
    if (component_count > 1 && ^"absrel.error_sink" == "Yes") {
        error_fraction_name = "error_sink_fraction";
        // Add local param for the last category's fraction
        model.generic.AddLocal (
            bs_rel,
            error_fraction_name,
            terms.AddCategory (utility.getGlobalValue("terms.mixture.mixture_aux_weight"), component_count)
        ...
        // Force [0, 0.01]
        parameters.SetRange (error_fraction_name,
            {
                lower_bound: "0",
                upper_bound: "0.01"
            }
    ...
    /*******************************************************************
     3) Now we define each category from 1..component_count - 1
        using 'partial' stick-breaking, and the final category uses
        'error_sink_fraction'.
    ********************************************************************/
    for (component = 1; component <= component_count; component += 1) {
        ...
        //----------------------------------------------------------------
        // Decide if this is the error sink (LAST component) or not
        //----------------------------------------------------------------
        if (component == component_count && ^"absrel.error_sink" == "Yes") {

            // Use 'omega_error' instead of 'omega`component`'
            ExecuteCommands ("
                function rate_generator (fromChar, toChar, namespace, model_type, model) {
                    return absrel.BS_REL._GenerateRate (
                       fromChar, toChar,
                       namespace,
                       model_type,
                       model[utility.getGlobalValue('terms.translation_table')],
                       ...
                       'alpha',       utility.getGlobalValue('terms.parameters.synonymous_rate'),
                       'beta_error',  terms.AddCategory(utility.getGlobalValue('terms.parameters.nonsynonymous_rate'), component),
                       'omega_error', terms.AddCategory(utility.getGlobalValue('terms.parameters.omega_ratio'), component)
                    );
                }
            ");
        ...
        //----------------------------------------------------------------
        // If NOT the last AND not the second-last in this scheme,
        // attach a normal stick-break param
        //----------------------------------------------------------------
        if (component < component_count - 1) {
            // i.e. categories 1..(component_count-2)
            model.generic.AddLocal (
                bs_rel,
                _aux[component - 1],
                terms.AddCategory (utility.getGlobalValue("terms.mixture.mixture_aux_weight"), component)
            ...
            // The mixture weight is from the partial stick-breaking
            (bs_rel [^'terms.mixture.mixture_components'])[key] = _wts [component - 1];

        } else if (component == (component_count - 1)) {
            //----------------------------------------------------------------
            // The second-last category
            // We do leftover = 1 - error_sink_fraction - sum of earlier
            //----------------------------------------------------------------
            //  We can do a small function that references the partial break:
            leftoverName = "partial_leftover_for_component_" + component;
            model.generic.AddLocal (
                bs_rel,
                leftoverName,
                terms.AddCategory (utility.getGlobalValue("terms.mixture.mixture_aux_weight"), component)
            ...
            // We'll define a custom function to compute leftover:
            ExecuteCommands ("
                function " + leftoverName + "() {
                    // sum up the partial stick-break for previous categories
                    sumSoFar = 0;
                    for (i = 0; i < " + (component - 1) + "; i += 1) {
                        sumSoFar += ^" + _aux[0] + "[i];
                    }
                    if (^'absrel.error_sink' == 'Yes') {
                        return 1 - ^" + error_fraction_name + " - sumSoFar;
                    } else {
                        return 1 - sumSoFar; // fallback if no error sink
                    }
                }
            ");

            // Force leftover to be >=0
            parameters.SetConstraint(leftoverName, leftoverName, "");

            (bs_rel [^'terms.mixture.mixture_components'])[key] = ^leftoverName;

        } else if (component == component_count && ^"absrel.error_sink" == "Yes") {
            // The final category uses the small fraction param
            (bs_rel [^'terms.mixture.mixture_components'])[key] = ^error_fraction_name;
        } else {
            // If no error sink at all, or final w/o error sink 
            // => leftover from stick-breaking
            if (component_count == 1) {
                // Only 1 category total => forced proportion = 1
                (bs_rel [^'terms.mixture.mixture_components'])[key] = 1;
            } else {
                // leftover
                (bs_rel [^'terms.mixture.mixture_components'])[key] = _wts [component - 1];
        ...
        //----------------------------------------------------------------
        // If it's the error sink, also constrain omega_error to [100,1e10]
        //----------------------------------------------------------------
        if (component == component_count && ^"absrel.error_sink" == "Yes") {
            error_omega_id = terms.AddCategory(utility.getGlobalValue('terms.parameters.omega_ratio'), component);
            parameters.SetRange(
    ...
    // Finally, set the final mixture as a dictionary of Q matrices
    bs_rel[utility.getGlobalValue("terms.model.rate_matrix")] = rate_matrices;

    // Typically fix A->G rate to 1
    parameters.SetConstraint(
        ((bs_rel[utility.getGlobalValue("terms.parameters")])[utility.getGlobalValue("terms.global")])[terms.nucleotideRate("A","G")],
        "1", 
        ""
    );
